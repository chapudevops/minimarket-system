        ...
        <div class="card">
            <div class="card-header">
                <div class="row align-items-center">
                    <div class="col">
                        <h4 class="mb-0">
                            <i class="bi bi-file-text-fill"></i> Nueva Nota de Venta
                        </h4>
                        <p class="mb-0 text-muted small">Registra una nueva nota de venta electrónica</p>
                    </div>
...
$(document).ready(function() {
    ...
    function renderizarProductos() {
            ...
            productosAgregados.forEach((item, idx) => {
                ...
                html += `
                    <div class="product-row" data-index="${idx}">
                        ...
                        <div class="row mt-2">
                            <div class="col-12">
                                <small class="text-muted">Total: <strong class="producto-total">S/ ${totalItem.toFixed(2)}</strong></small>
                            </div>
                        </div>
                    </div>
                `;
        ...
        $('.remove-product').off('click').on('click', function() {
            ...
            Swal.fire({
                ...
                text: `¿Desea eliminar "${productosAgregados[idx].descripcion}" de la lista?`,
                icon: 'warning',
                        ...
                        response.productos.forEach(producto => {
                            ...
                            html += `
                                <div class="product-result-item" 
                                    ...
                                    <small class="text-muted">Precio: S/ ${precio.toFixed(2)} | Unidad: ${producto.unidad || 'UNIDAD'}</small>
                                </div>
                            `;
                        });
                        ...
                        $('.product-result-item').off('click').on('click', function() {
                            ...
                            if (productosAgregados.some(p => p.id === id)) {
                                Swal.fire({
                                    ...
                                    text: 'Este producto ya está agregado a la nota',
                                    confirmButtonColor: '#3085d6'
                                });
                                return;
                            }
    ...
    $('#btnGuardar').click(function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    Swal.fire({
                        ...
                        text: response.message || 'La nota de venta ha sido creada exitosamente',
                        confirmButtonColor: '#3085d6'
                    }).then(() => {
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                        text: response.message || 'Error al guardar la nota de venta'
                    });
                    $('#btnGuardar').show();
                    $('#btnLoading').hide();
                }
            ...
            error: function(xhr) {
                let errorMsg = 'Error al guardar la nota de venta';
                try {
