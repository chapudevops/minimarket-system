...
<head>
    <meta charset="UTF-8">
    <title>Comprobante de Venta - {{ $venta->documento }}</title>
    <style>
...
<body>
    <div class="header">
        <h1>{{ $empresa->razon_social ?? 'DISTRIBUIDORA BEJAR E.I.R.L.' }}</h1>
        <p>{{ $empresa->direccion ?? 'MZA. E LOTE. 2 CAS. SAN MARTIN ICA - ICA - ICA' }}</p>
        <p>RUC: {{ $empresa->ruc ?? '20100066603' }}</p>
    </div>

    <div class="info-section">
        <table>
            <tr><td width="20%"><strong>Documento:</strong></td><td width="30%">{{ $venta->documento }}</td>
                <td width="20%"><strong>Fecha:</strong></td><td width="30%">{{ $venta->fecha_emision->format('d/m/Y H:i:s') }}</td></tr>
            <tr><td><strong>Cliente:</strong></td><td>{{ $venta->cliente->nombre_razon_social ?? 'CLIENTES VARIOS' }}</td>
                <td><strong>RUC/DNI:</strong></td><td>{{ $venta->cliente->numero_documento ?? '00000000' }}</td></tr>
            <tr><td><strong>Dirección:</strong></td><td colspan="3">{{ $venta->cliente->direccion ?? '-' }}</td></tr>
        </table>
    </div>
    ...
    <div class="total-section">
        <table>
            <tr><td>OP. Gravadas:</td><td class="text-right">S/ {{ number_format($venta->subtotal, 2) }}</td></tr>
            <tr><td>IGV (18%):</td><td class="text-right">S/ {{ number_format($venta->igv, 2) }}</td></tr>
            <tr class="total-row"><td><strong>TOTAL:</strong></td><td class="text-right"><strong>S/ {{ number_format($venta->total, 2) }}</strong></td></tr>
        </table>
    </div>
